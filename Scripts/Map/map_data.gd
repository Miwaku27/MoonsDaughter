...
func _init(map_width: int, map_height: int, _player: Player) -> void:
	width = map_width
	height = map_height
	player = _player
	entities = []
...
func restore(save_data: Dictionary) -> void:
	...
	for entity_data in save_data["entities"]:
		...
		if entity_data.has("facing_direction"):
			player.restore(entity_data)
			new_entity = player
		else:
			new_entity = EntityManager.create_entity(self, Vector2i.ZERO, "")
