...
const inventory_menu_item_scene := preload("res://Scenes/GUI/inventory_menu_item.tscn")
...
func _register_item(index: int, item: Entity, is_equipped: bool) -> void:
	...
	item_button.text = "( %s ) %s" % [index, item.get_entity_name()]
	if is_equipped:
		item_button.text += " (E)"
	var shortcut_event := InputEventKey.new()
	@warning_ignore("int_as_enum_without_cast")
	shortcut_event.keycode = KEY_0 + index
...
func _physics_process(_delta: float) -> void:
	if Input.is_action_just_pressed("cancel") or Input.is_action_just_pressed("inventory"):
