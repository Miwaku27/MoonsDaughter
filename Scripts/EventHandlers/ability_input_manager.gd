...
var player_abilities: Array[String] = ["attack", "faraway", "aoe"]
...
func select_ability(i: int) -> Action:
	if player_abilities.size() > i:
		...
		if current_ability and abi == current_ability.id:
			return start_ability()
		else:
			unselect_ability()
			current_ability = Database.abilities[abi]
	return null
...
func start_ability() -> Action:
	...
	var action = AbilityAction.new(player, current_ability, selected_tiles)
	unselect_ability()
	return action
